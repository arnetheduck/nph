...
#   - postfix is the dustbin that anything that the parser doesn't know where to
#     place ends up - these are rendered after the AST node which is the most
#     common cause for comment reordering, ie that we didn't find a suitable
#     place to put them
#
# Comment handling is made difficult because the upstream parser "sometimes"
# puts comments in the AST and the rules for this have evolved over time meaning
# that great creativity exists in where comments may appear. It doesn't help
...
proc splitComments(
  ...
  for c in comments:
    if c.indent == -1 or c.indent > ind:
      a.addComment(c, commentLoc)
    else:
      result.add(c)
...
proc parseImport(p: var Parser; kind: TNodeKind): PNode =
  ...
  p.skipped = commentLookahead(p) # TODO `mid` for import?
...
proc parseIncludeStmt(p: var Parser): PNode =
  ...
  p.skipped = commentLookahead(p) # TODO `mid` for include?
  while true:
    # was: while p.tok.tokType notin {tkEof, tkSad, tkDed}:
    p.hasProgress = false
...
proc parseObject(p: var Parser): PNode =
  ...
  if not realInd(p):
    ...
    p.skipped = splitComments(p.skipped, result, p.tok.indent - 1, clMid)
...
proc parseTypeDef(p: var Parser): PNode =
  ...
  var comments = commentLookahead(p)
  if p.tok.tokType == tkBracketLe and p.validInd:
    p.skipped = splitComments(comments, identifier, p.tok.indent, clPostfix)
    genericParam = parseGenericParamList(p)
    comments = commentLookahead(p)
    next = genericParam
  else:
    genericParam = p.emptyNode
  # pragma = optPragmas(p)
  if p.tok.tokType == tkCurlyDotLe and (p.tok.indent < 0 or realInd(p)):
    p.skipped = splitComments(comments, next, p.tok.indent, clPostfix)
  ...
  if p.tok.tokType == tkEquals:
    ...
    comments = commentLookahead(p)
    p.skipped = splitComments(comments, result[^1], p.currInd, clPostfix)
  else:
    result.add(p.emptyNode)
  ...
  if p.tok.indent > p.currInd:
    ...
    withInd(p):
      ...
      while true:
        ...
        let comments = commentLookahead(p)

        p.skipped.add splitComments(comments, a, p.currInd, clPostfix)
...
proc parseAll(p: var Parser): PNode =
  ...
  while p.tok.tokType != tkEof:
    ...
    if a.kind != nkEmpty and p.hasProgress:
      ...
      getTok(p)
    if p.tok.indent != 0 and p.tok.tokType != tkComment:
      debugEcho p.tok.tokType, " ", p.tok.indent

      parMessage(p, errInvalidIndentation % "parseAll")
  for c in p.skipped:
    let n = newNodeI(nkCommentStmt, getLineInfo(p.lex, c))
